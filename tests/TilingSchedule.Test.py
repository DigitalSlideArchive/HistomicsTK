...
#reads in all tiles, shrinks them significantly, and recomposes the original image.
#The re-composed image should appear coherent.
...
#re-compose image by grabbing each tile in the schedule, and resizing it into a list
Image = numpy.zeros((Tshrink*Schedule.X.shape[0], Tshrink*Schedule.X.shape[1],  4), dtype=numpy.uint8)
for i in range(Schedule.X.shape[0]):
    for j in range(Schedule.X.shape[1]):
        
        #read region
        Region = Slide.read_region((int(Schedule.X[i,j]), int(Schedule.Y[i,j])), Schedule.Level, (Schedule.Tout, Schedule.Tout))
        ...
        Image[i*Tshrink:(i+1)*Tshrink, j*Tshrink:(j+1)*Tshrink, :] = Region
