...
def _compute_marginal_glcm_probs_cython(double[:, :] nGLCM not None):
    ...
    cdef double[::1] px = np.zeros([num_levels], dtype=np.float)
    cdef double[::1] py = np.zeros([num_levels], dtype=np.float)

    cdef double[::1] pxPlusy = np.zeros([2*num_levels-1], dtype=np.float)
    cdef double[::1] pxMinusy = np.zeros([num_levels], dtype=np.float)
