...
def MinimumModel(I, Delta=0.3, MaxLength=255, Compaction=3,
    ...
    Label = htk.CompactLabel(Label, Compaction)
...
def TraceContours(I, X, Y, Min, Max, MaxLength=255):
    ...
    for i in np.arange(X.size):
        ...
        cX, cY = htk.TraceBounds(Embed, Connectivity=4,
...
def SplitConcavities(Label, MinDepth=4, MinConcavity=np.inf):
    ...
    if np.unique(Convex).size-1 != Convex.max():
        Convex = htk.CondenseLabel(Convex)
    ...
    while i <= Total:
        ...
        X, Y = htk.TraceBounds(Mask, Connectivity=8)
