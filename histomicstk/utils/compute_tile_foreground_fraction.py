...
def compute_tile_foreground_fraction(slide_path, im_fgnd_mask_lres,
                                     fgnd_seg_scale, tile_position=None,
                                     **it_kwargs):
    """
    ...
    if tile_position is None:
        ...
        tile_fgnd_frac = [None] * num_tiles

        for tile_position in range(num_tiles):

            tile_fgnd_frac[tile_position] = \
                dask.delayed(compute_tile_foreground_fraction)(
                    slide_path, im_fgnd_mask_lres, fgnd_seg_scale,
                    tile_position=tile_position,
                    **it_kwargs
                )

        tile_fgnd_frac = np.array(dask.delayed(tile_fgnd_frac).compute())
        ...
        raise ValueError('Invalid value for tile_position. Must be None or int')
